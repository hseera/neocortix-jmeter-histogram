        ...
        try: #extract data from csv files that begin with TestPlan_
            if file.startswith("TestPlan_") and file.endswith(".csv"):
                ...
                for item in range(len(x)):
                    #s, ms = divmod(x['timeStamp'].values[item], 1000)
                    fwrite.write('%s,%s,%s,%s\n' %(x['timeStamp'].values[item],"loc_"+substring,x['elapsed'].values[item],x['responseCode'].values[item]))
            else:
                continue
...
def generate_graphs():
    ...
    try:
        ...
        ax = sns.scatterplot(ax=axes[0,0], data=df,x=df['timestamp'],y=df['latency'], hue=df['location'], s=7, legend=True, palette="deep",style=df['location'])
        ax.set(ylim=(0,3000))
        ...
        kwargs = dict(element='step',shrink=.8, alpha=0.3, fill=True, legend=False) 
...
#        
        ...
        ax = sns.lineplot(ax=axes[1, 1],data=summary,dashes=False, palette="deep")
        ax.legend(fontsize='medium')
        ax.set(ylim=(0,6000))
        ...
        ax = sns.scatterplot(ax=axes[2, 0], data=df,x=df['timestamp'],y=df['responsecode'], hue=df['location'], s=10, legend=True,palette="deep",style=df['location'])
        ax.set(ylim=(0,600))
        ax.legend(fontsize='medium')
        ...
        ax = sns.barplot(ax=axes[2, 1],data=percents_df,x=0, y='responsecode', hue='location', orient = 'h' )
        ax.legend_.remove()
